...
document.getElementById("exportButton").addEventListener("click",async()=>{
  const button=document.getElementById("exportButton");const format=document.getElementById("exportFormat").value;button.disabled=true;button.textContent="Preparing…";
  try{const response=await fetch("/api/export",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...filters(),format})});if(!response.ok){const data=await response.json();throw new Error(data.error||"Report could not be created.")}const blob=await response.blob();const count=response.headers.get("X-Visit-Count")||"0";const link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download=`sos-visits-${form.fromDate.value}-to-${form.toDate.value}.${format}`;link.click();URL.revokeObjectURL(link.href);showMessage(`${format==="xlsx"?"Excel workbook":"CSV file"} downloaded: ${count} matching visit${count==="1"?"":"s"}.`)}catch(error){showMessage(error.message,"error")}finally{button.disabled=false;button.textContent="Download Report"}
});
