...
body.addEventListener("click",async event=>{
  ...
  if(event.target.closest(".save")){
    ...
    try{await request("update",{id:row.dataset.id,arrivalGuardianName:row.querySelector(".arrival-guardian").value.trim(),departureGuardianName:row.querySelector(".departure-guardian").value.trim(),clientFirstName:row.querySelector(".first").value.trim(),clientLastName:row.querySelector(".last").value.trim(),visitDate:row.querySelector(".date").value,arrivalTime:row.querySelector(".arrival").value,departureTime:row.querySelector(".departure").value});showMessage("Visit updated successfully.");await loadRecords()}catch(error){showMessage(error.message,"error");button.disabled=false}
  }
